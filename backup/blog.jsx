...
const BlogPost = ({ post }) => {
  ...
  return (
    <Link to={`blog${post.node.fields.slug}`} className="BlogPost">
      <h3 className="BlogPost__title">{frontmatter.title}</h3>
      <div className="BlogPost__categories">Web Development, Tutorial</div>
      <div className="BlogPost__description">{frontmatter.description}</div>
    </Link>
  )
}
